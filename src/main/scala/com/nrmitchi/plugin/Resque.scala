package com.nrmitchi.plugin


import net.greghaines.jesque._
import net.greghaines.jesque.client.ClientImpl
import play.Logger
import tests._
import tests.TestAction
import scala.collection.JavaConversions._
import java.util.Arrays;
import net.greghaines.jesque.worker.JobFactory;
import net.greghaines.jesque.worker.Worker;
import net.greghaines.jesque.worker.WorkerImpl;
import net.greghaines.jesque.worker.MapBasedJobFactory;
import java.util.Date
import play.api.libs.json.{JsObject, JsString, Json}


object Resque {
  private var _host: String        = _
  private var _port: Int           = _
  private var _database: Int       = _
  private var _namespace: String   = _
  private var _password: Option[String]    = _
  private var _debug: Boolean      = _

  private lazy val config1 = new ConfigBuilder()
                                  .withHost(_host)
                                  .withPort(_port)
                                  .withDatabase(_database)
                                  .withPassword(_password match {
                                    case Some(x) => if (x.length <= 0) null else x
                                    case None => null
                                  })
                                  .withNamespace( if (_namespace.length <= 0) "play2resque" else _namespace )



  val config = new ConfigBuilder().withHost("localhost")
                                  .withPort(6379)
                                  .withDatabase(1)
                                  .withPassword(null)
                                  .withNamespace("resque")

  private lazy val jesque = new ClientImpl(config.build())

  def apply(host: String, port: Int, database: Int, namespace: String, password: Option[String], debug: Boolean) = {
    _host = host
    _port = port
    _namespace = namespace
    _database = database
    _password = password

    jesque
  }

  private def log(x: Any) {
    if (_debug) Logger.debug("Resque:: "+x.toString)
  }
  def close() {
    log("Closing Resque connections...")
    try {
      jesque.end
    } catch {
      case e: Exception => log("Exception closing Jesque: "+e.getMessage)
    }
    log("Resque connections closed...")
  }

  def withClient[T](body: ClientImpl => T) = {
    /*
     * I feel like I should be passing in a client, insead of jesque itself
     *
     * eg.
     *
     * jesque.withClient {
     *   body
     * }
     */
    body(jesque)

  }

  // Shortcut for publishing an event
  def push(queue: String, className: String, payload: java.lang.Object*) = {
    jesque.enqueue(queue, new Job(className, payload.toSeq:_*))
  }

  def create_jobs = {
    jesque.enqueue("hello", new Job("TestAction", "params","moreparams"))
    jesque.enqueue("hello", new Job("TestAction1", "params", "moreparams"))
  }

  def worker_try:Thread = {
    val worker = new WorkerImpl(config.build(), Arrays.asList("hello"), new MapBasedJobFactory(
      mapAsJavaMap(scala.collection.immutable.Map("TestAction" -> classOf[TestAction]))
    ).asInstanceOf[JobFactory])
   
    new Thread(worker)
  }
  def worker_try1:Thread = {
    val worker = new WorkerImpl(config.build(), Arrays.asList("hello"), new MapBasedJobFactory(
      mapAsJavaMap(scala.collection.immutable.Map("TestAction1" -> classOf[TestAction1]))
    ).asInstanceOf[JobFactory])
    new Thread(worker)
  }

  def start_worker(worker: Thread) =   worker.start()
  def stop_worker(worker: Thread) = worker.stop()

}



case class TestAction1(i:String,d:String) extends Runnable {

   override def run() {
      println("Job ran at=")
   }
}